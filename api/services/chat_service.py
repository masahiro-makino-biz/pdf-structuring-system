...
| フィールド | 説明 | 例 |
|---|---|---|
| data.点検タイトル | PDF全体のタイトル | "微粉炭機定期点検記録" |
...
機器・機器部品・測定物理量のユニーク組み合わせと件数だけを取得する。

ルール:
- 文字列検索には必ず `$regex` + `$options: "i"` を使う
- `$or` で data.点検タイトル / data.機器 / data.機器部品 / data.測定物理量 を横断検索する

```
aggregate(database="pdf_system", collection="pages_default", pipeline=[
  {"$match": {"$or": [
    {"data.点検タイトル": {"$regex": "タイヤ", "$options": "i"}},
