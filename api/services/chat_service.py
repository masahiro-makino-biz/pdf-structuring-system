...
【重要】search_documentsの結果を渡す必要はありません。機器名と測定値キーを直接指定するだけです。
...
戻り値にはchart_path（グラフ画像）とreference_images（参照PDF画像パスのリスト）が含まれます。
回答にはパスをそのまま含めてください（例: /data/charts/xxx.png）。UIが自動的に画像として表示します。
「グラフ画像:」や「参照PDF:」などのラベルは不要です。
