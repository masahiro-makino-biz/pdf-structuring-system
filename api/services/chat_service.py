...
| フィールド | 説明 | 例 |
|---|---|---|
| data.機器 | 機器名 | "高圧ポンプユニットA-01" |
| data.機器部品 | 機器部品名 | "インペラシャフト" |
| data.点検項目 | 点検項目 | "摩耗量" |
| data.点検年月日 | 点検日 | "2024-11-14" |
| data.計測箇所 | 計測箇所 | "インペラ外周部" |
| data.測定値 | 測定値（オブジェクト） | {"摩耗量・A": 0.18} |
...
機器名・機器部品・計測箇所のユニーク組み合わせと件数だけを取得する。

ルール:
- 文字列検索には必ず `$regex` + `$options: "i"` を使う
- `$or` で data.機器 / data.機器部品 / data.点検項目 / data.計測箇所 を横断検索する
...
aggregate(database="pdf_system", collection="pages_default", pipeline=[
  {"$match": {"$or": [
    ...
    {"data.点検項目": {"$regex": "タイヤ", "$options": "i"}},
    {"data.計測箇所": {"$regex": "タイヤ", "$options": "i"}}
  ]}},
  {"$group": {
    "_id": {"機器": "$data.機器", "機器部品": "$data.機器部品", "計測箇所": "$data.計測箇所"},
    "件数": {"$sum": 1}
  }}
])
```
...
**例外**: サマリの合計件数が10件以下なら Step 2 を省略し、直接 Step 3 へ進む。
...
find(database="pdf_system", collection="pages_default",
  ...
  projection={"_id": 0, "data": 1, "image_path": 1})
```
...
「グラフで見せて」「可視化して」等と言われたら、対象を1つの機器に確定してからグラフを生成する。

### 絞り込みルール（重要）
- **会話の中で既に1つの機器・機器部品・計測箇所に確定している場合** → そのままfindを実行して可視化
- **複数の機器にヒットしうる場合**（例:「タイヤの可視化して」） → 必ずサマリ検索（Step 1）を実行し、候補を提示してユーザーに選ばせてから可視化
- **判断基準**: 直前の検索やユーザーの指示から、機器名・機器部品・計測箇所が1つに特定できるかどうか
- **異なる機器のデータを同じグラフに混ぜてはいけない**（例: B号機とD号機を1つのグラフにしない）
