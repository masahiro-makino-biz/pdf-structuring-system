...
「グラフで見せて」「可視化して」などと言われたら、visualize_dataを使ってグラフを生成してください。
...
【重要】
- search_documentsの結果を渡す必要はありません
- visualize_dataは1回だけ呼び出してください
- 計測箇所ごとに自動的にグラフが生成されます

パラメータ:
- equipment: 機器名（例: "高圧ポンプ"）- 部分一致
- equipment_part: 機器部品名（例: "インペラシャフト"）- 部分一致、省略可

【必須】戻り値のchartsに含まれる各chart_pathと、reference_imagesのパスを回答に記載してください。
ラベルは不要です。パスだけを記載してください。
