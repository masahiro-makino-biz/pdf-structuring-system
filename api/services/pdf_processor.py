        ...
        "properties": {
            "records": {
                ...
                "description": (
                    "個々の点検記録を要素として持つ配列。"
                    "1ページに複数の表がある場合は複数要素。"
                    "1つの表に複数の物理量（摩耗量・振動値・温度 等）が並ぶ場合は"
                    "物理量ごとにレコードを分割して複数要素にする。"
                ),
                "items": {
                    "type": "object",
                    "additionalProperties": False,
...
def extract_page_data(image_path: str, page_number: int = 1) -> dict:
    ...
    system_prompt = (
        "あなたは発電所等の点検記録画像から、指定のJSONスキーマに厳密準拠したJSONを出力する専門家です。\n"
        "出力はJSONのみ（説明・コードフェンス禁止）。\n"
        "・トップはobject。\n"
        "・'records' は1ページから読み取れるレコード配列。\n"
    ...
    user_prompt = (
        ...
        "\n"
        "2) 表の中身（行と列のラベル＋セルの値）は全て 測定値 に入れる：\n"
        "   - キーは行・列のラベルをそのまま使う\n"
        "   - 複数階層のラベルは『・』区切り（例：『A点・上』『1段・左』）\n"
        "   - 物理量（摩耗量/振動値 等）は測定物理量フィールドに移すのでキーには含めない\n"
        "\n"
        "3) 1つの表に複数の物理量（摩耗量・振動値・温度 等）が並ぶ場合：\n"
        "   - 物理量ごとにレコードを分割する（1物理量 = 1レコード）\n"
        "   - 測定物理量を『摩耗量』や『振動値』など個別の値にして、それぞれのレコードを出力\n"
        "\n"
        "4) 基準値は 基準値 に格納する。単一なら物理量名をキー、行ごとに違えば測定値と同じキー形式。\n"
        "\n"
        ...
        "   表の内容から推測してよい（例：測定値が微小なmm値なら『摩耗量』と推測）。\n"
        "\n"
        "8) ハルシネーションは厳禁。\n"
        "9) JSONのみ。余計な文章は一切禁止。\n"
        "10) 日付はYYYY-MM-DD形式。\n"
